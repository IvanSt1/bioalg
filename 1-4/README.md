...
На первом графике представлены две зависимости: время выполнения генетического алгоритма и максимальная приспособленность от размера популяции. Давайте подробно рассмотрим каждую из них:

Зависимость времени выполнения от размера популяции (верхний график, синий цвет):
...
Зависимость максимальной приспособленности от размера популяции (нижний график, красный цвет):
...
Верхний график показывает, что время выполнения растёт линейно с увеличением количества родителей. Это может быть связано с тем, что больше родителей участвуют в создании потомства, что требует больше операций кроссовера.

Нижний график показывает зависимость максимальной приспособленности от количества родителей. Из графика видно, что приспособленность увеличивается с количеством родителей до определённой точки, после которой начинает ухудшаться. Это может указывать на то, что слишком большое количество родителей ведёт к избыточному разнообразию, которое может размывать полезные генетические комбинации.

### График зависимости от количетсва поколений (res3.png)

![количество поколений](./res2.png)

На верхнем графике показана зависимость времени выполнения от количества поколений. Время выполнения увеличивается по мере увеличения числа поколений, что логично, так как требуется больше итераций алгоритма.

Нижний график показывает зависимость максимальной приспособленности от количества поколений. Приспособленность растёт с увеличением количества поколений, что указывает на то, что алгоритм успевает лучше исследовать пространство решений и находить более оптимальные решения.
